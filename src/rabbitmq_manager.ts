...
import RabbitConnection from './rabbitmq_connection.js'
import safeStringify from './safe_stringify.js'
import { MessageContract, RabbitConfig } from './types.js'
...
export class RabbitManager {
   ...
   * @param options The options
   */
  public async sendToQueue(queueName: string, content: any, options?: Options.Publish) {
    const channel = await this.getChannel()
   ...
   * @param content The content
   */
  public async sendToExchange(exchangeName: string, routingKey: string, content: any) {
    const channel = await this.getChannel()
  ...
  public async validateConnection() {
    try {
      ...
      const channelPromise = connection.createChannel() as unknown as Promise<Channel>
      ...
      const hasConnection = await this.rabbitConnection.createConnection()
